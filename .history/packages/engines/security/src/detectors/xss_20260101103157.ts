...
export function detectXSS(ast: ASTNode, filePath: string): Detection[] {
  ...
  function visit(node: ASTNode): void {
    ...
    if (node.type === 'MemberExpression' && node.children) {
      ...
      if (propertyNode && dangerousPatterns.some((pattern) => pattern.includes(String(propertyNode.value)))) {
        detections.push({
          ...
          severity: 'high',
          category: 'security',
          message: `Potential XSS vulnerability: unsafe use of ${propertyNode.value}`,
          metadata: {
            pattern: 'xss-dom-manipulation',
            property: String(propertyNode.value),
            explanation:
    ...
    if (
      ...
      node.children?.some((child) => child.value === 'dangerouslySetInnerHTML')
    ) {
      detections.push({
        id: `xss-${++detectionCounter}`,
        ruleId: 'xss-react',
