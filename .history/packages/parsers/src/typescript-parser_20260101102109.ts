  ...
  for (const key in node) {
    ...
    if (value && typeof value === 'object') {
      ...
      if (value.type && value.loc) {
        children.push(convertESTreeNode(value as TSESTree.Node))
      }
      // Array of child nodes
      else if (Array.isArray(value)) {
        for (const item of value) {
          if (item && typeof item === 'object' && item.type && item.loc) {
            children.push(convertESTreeNode(item as TSESTree.Node))
          }
        }
      }
    }
