...
import { type Detection, type RivetConfig, RivetEngine, type Severity } from '@rivet/core'
...
export const fixCommand = new Command('fix')
  ...
  .option(
    '--severity <level>',
    'Minimum severity level to fix (critical, high, medium, low, info)',
    'info'
  )
  ...
  .action(async (targetPath: string, options: Record<string, string | boolean>) => {
    ...
    try {
      ...
      spinner.succeed(
        `Found ${fixableDetections.length} fixable issues out of ${result.detections.length} total`
      )
      console.log('')
      ...
      // Group by file
      const fileGroups = fixableDetections.reduce<Record<string, Detection[]>>((acc, d) => {
      ...
      if (options.dryRun) {
        ...
        console.log(
          chalk.cyan(
            `Would fix ${fixableDetections.length} issues in ${Object.keys(fileGroups).length} files`
          )
        )
      } else {
        ...
        for (const [filePath, detections] of Object.entries(fileGroups)) {
          try {
            ...
            fixSpinner.warn(
              `Failed to fix ${filePath}: ${error instanceof Error ? error.message : error}`
            )
          }
        }
