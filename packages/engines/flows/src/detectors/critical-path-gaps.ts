      ...
      if (body && !hasTryCatch(body) && node.loc) {
        detections.push({
          ...
          metadata: {
            ...
            explanation:
              'Async operations can fail and should have proper error handling to prevent unhandled rejections',
    ...
    if (parent.type === 'MemberExpression' && parent.raw.type === 'MemberExpression') {
      ...
      if (property && property.name === 'then') {
        ...
        if (grandparent?.type === 'CallExpression' && grandparent.children) {
          const args = grandparent.children.filter(
            (c) => c.type !== 'MemberExpression' && c.type !== 'Identifier'
          )
          if (args.length >= 2) {
